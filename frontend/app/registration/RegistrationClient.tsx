...
export default function RegistrationClient() {
    ...
    (async () => {
      try {
        ...
        setParent({
          ...
          role: String(data.parent?.roleId ?? "").trim(),
        });
  ...
  function validate(): boolean {
    ...
    setParentErr(pe);
    return !Object.values(pe).some(Boolean);
  }
        ...
        <main className="govuk-main-wrapper auth-center" id="main-content" role="main">
          <div className="auth-form">
            ...
            <p className="govuk-error-message">
              <span className="govuk-visually-hidden">Chyba:</span> {prefillError}
            </p>
            ...
            <form noValidate onSubmit={handleFinish}>
              <fieldset className="govuk-fieldset">
                ...
                <div className={`govuk-form-group ${parentErr.firstName ? "govuk-form-group--error" : ""}`}>
                  <label className="govuk-label" htmlFor="parent-firstName">
                    Meno
                  </label>
                  {parentErr.firstName && (
                    <p className="govuk-error-message">
                      <span className="govuk-visually-hidden">Chyba:</span> {parentErr.firstName}
                    </p>
                  )}
                  <input
                ...
                <div className={`govuk-form-group ${parentErr.lastName ? "govuk-form-group--error" : ""}`}>
                  <label className="govuk-label" htmlFor="parent-lastName">
                    Priezvisko
                  </label>
                  {parentErr.lastName && (
                    <p className="govuk-error-message">
                      <span className="govuk-visually-hidden">Chyba:</span> {parentErr.lastName}
                    </p>
                  )}
                  <input
                ...
                <div className={`govuk-form-group ${parentErr.email ? "govuk-form-group--error" : ""}`}>
                  <label className="govuk-label" htmlFor="parent-email">
                    E-mail
                  </label>
                  <span className="govuk-hint">E-mail je daný pozvánkou a nedá sa zmeniť.</span>
                  <input className="govuk-input govuk-input--width-20" id="parent-email" type="email" value={parent.email} disabled />
                </div>

                <div className={`govuk-form-group ${parentErr.phone ? "govuk-form-group--error" : ""}`}>
                  <label className="govuk-label" htmlFor="parent-phone">
                    Telefón
                  </label>
                  <span className="govuk-hint">Formát: +421 9xx xxx xxx</span>
                  {parentErr.phone && (
                    <p className="govuk-error-message">
                      <span className="govuk-visually-hidden">Chyba:</span> {parentErr.phone}
                    </p>
                  )}
                  <input
                    ...
                    return (
                      <div key={c.id} className="govuk-inset-text">
                        ...
                        <div className="govuk-form-group">
                          <label className="govuk-label" htmlFor={`${base}-firstName`}>
                            Meno
                          </label>
                          <input className="govuk-input govuk-input--width-20" id={`${base}-firstName`} value={c.firstName} readOnly />
                        </div>

                        <div className="govuk-form-group">
                          <label className="govuk-label" htmlFor={`${base}-lastName`}>
                            Priezvisko
                          </label>
                          <input className="govuk-input govuk-input--width-20" id={`${base}-lastName`} value={c.lastName} readOnly />
                        </div>
                      </div>
                    );
                  })}
